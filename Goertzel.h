...
static Complex Goertzel(Complex input[], uint16_t k);
static void Goertzel_FT(Complex output[],Complex input[]);
static double Goertzel_get_mag(Complex input[], uint16_t smpl_freq, uint16_t test_freq);
...
static Complex Goertzel(Complex input[], uint16_t k)
{
    ...
    sequence[0] = input[0].Re;
    sequence[1] = 2*cos(2*PI*k/N)*sequence[0] + input[1].Re;

    for(n =2; n < N; ++n)
    {
        sequence[n] = 2*cos(2*PI*k/N)*sequence[n-1] - sequence[n-2] + input[n].Re;
